 ...
 *
 * Debug shortcut: Ctrl+Shift+K to force-enable on touch devices.
 */
...
(function () {
    ...
    // Auto-init on non-touch devices
    if (!isTouch) initCursor();

    // Debug shortcut: Ctrl+Shift+K to force-enable cursor
    document.addEventListener('keydown', (e) => {
        if (e.ctrlKey && e.shiftKey && e.key === 'K') {
            e.preventDefault();
            if (!initialized) {
                initCursor();
                console.log('[cursor] Force-enabled via Ctrl+Shift+K');
            } else {
                // Toggle visibility if already initialized
                document.documentElement.classList.toggle('custom-cursor-active');
                const active = document.documentElement.classList.contains('custom-cursor-active');
                console.log(`[cursor] ${active ? 'Enabled' : 'Disabled'} via Ctrl+Shift+K`);
            }
        }
    });
